  ...
  _createClass(RTCPConnect, [{
    ...
    value: function createConnection(uid) {
      ...
      connection.ondatachannel = function (event) {
        return _this._receiveChannelCallback(event, connToUid);
      };

      if (this.uid !== connToUid) {
        connection.onicecandidate = function (event) {
          return _this._onIceCandidate(event, uid, connFromUid, connToUid);
        };
      }
    ...
    value: function _bindChannelEvents(channel) {
      ...
      channel.onopen = function () {
        return _this4._onSendChannelStateChange(channel);
      };
        ...
        if (typeof event.data === 'string') {
          if (event.data.indexOf('__fileDescription') > -1) {
            ...
            if (channel._receiveBuffer) {
              ...
              _this4.dropConnection(_filePeer);
            }
          } else {
            Sync.trigger('message', {
              type: 'text',
...
var linkifyStr = __webpack_require__(23);
...
var App = function () {
  ...
  function send(text) {
    _.map(wrtc.peers, function (peer) {
  ...
  function sendFile(file) {
    ...
    function createFileConnections(cb) {
      _.map(wrtc.peers, function (peers, key) {
    ...
    function closeFileConnections() {
      _.map(wrtc.peers, function (peer, key) {
        if (key.indexOf('_file') > -1) wrtc.dropConnection(key);
      });
    }

    function sendTransferPrepareInfo() {
      return _.map(wrtc.peers, function (peer, key) {

...
var RTCPConnect = (function(window) {
  ...
  function createConnection(toUid) {
    var userColour = underscore.sample(underscore.difference(colours, usedColours));
...
var textTemplate = function(data) {
var __t, __p = '';
__p += '<li class="message" style="color: ' +
((__t = ( data.colour )) == null ? '' : __t) +
'">' +
...
var fileTemplate = function(data) {
var __t, __p = '';
__p += '<li class="message" style="color: ' +
((__t = ( data.colour )) == null ? '' : __t) +
'">\n  <a download="' +
((__t = ( data.fileDescription.name )) == null ? '' : __t) +
'" href="' +
((__t = ( url )) == null ? '' : __t) +
'">\n    Received file "' +
((__t = ( data.fileDescription.name )) == null ? '' : __t) +
'" (' +
((__t = ( bytes(data.fileDescription.size) )) == null ? '' : __t) +
')\n  </a>\n</li>\n';
return __p
};
...
bytes_1$1.format = format_1;
bytes_1$1.parse = parse_1;
...
var App = (function(window) {
  ...
  function init(connectionId, peers, history) {
    ...
    RTCPConnect.init(connectionId, peers, history);
    ...
    ws.onmessage = function(event) {
      ...
      switch (message.type) {
        case 'newUser':
          ...
          RTCPConnect.createConnection(uid);
          // create channels
          RTCPConnect.createChannel(uid);
          // send offer
          RTCPConnect.createOffer(uid);
          break;

        case 'offerFrom':
          RTCPConnect.handleOffer(message);
          break;

        case 'answerFrom':
          RTCPConnect.handleAnswer(message);
          break;

        case 'iceCandidateFrom':
          RTCPConnect.handleIceCandidate(message);
          break;

        case 'channelClose':
          RTCPConnect.dropConnection(message.uid);
      }
    };

    window.addEventListener('beforeunload', function() {
      Sync.trigger('channelCloseWS', RTCPConnect.getUid());
      return null;
    });
  }
  ...
  function sendMessage(text) {
    RTCPConnect.getPeers().each(function(peer) {
      var channel = peer.get('channel');
      if (channel && channel.readyState === 'open') channel.send(_str({
        fromUid: RTCPConnect.getUid(),
        type: 'text',
        data: text,
      }));
    });
  ...
  function _createFileChannels(next) {
    RTCPConnect.getPeers().each(function(peer) {
      var key = peer.get('id');
      var channel = RTCPConnect.createChannel(key, key + '_channel_file');
    ...
    reader.onload = (function(_file) {
      return function(e) {
        ...
        if (_file.size > options.offset + e.target.result.byteLength) {
          ...
          messageHistoryUpdate({
            type: 'text',
            data: 'Sent file "' + _file.name + '" (' + bytes_1$1(_file.size) + ')',
            outgoing: true,
          });
...
var backbone_nativeview = createCommonjsModule(function (module) {
...
(function (factory) {
  if (typeof undefined === 'function' && undefined.amd) { undefined(['backbone'], factory);
  } else { module.exports = factory(backbone);
  }
}(function (Backbone) {
...
var MainView = backbone.View.extend({
  ...
  updateUsersList: function() {
    return this.usersList.innerHTML = this.users.map(function(user) {
      return new User({ model: user }).render().el.outerHTML;
    }).join('');
  },
...
var Users$1 = backbone.Collection.extend({
  model: User$1,
});
